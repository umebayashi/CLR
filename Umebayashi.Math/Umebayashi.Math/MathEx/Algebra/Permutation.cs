...
namespace Umebayashi.MathEx.Algebra
{
	...
	public class Permutation
	{
		...
		private Permutation(VectorI vector, VectorI[] elements)
		{
			this.Vector = vector;
		...
		/// <param name="size"></param>
		/// <returns></returns>
		public static Permutation[] Calculate(int size)
		{
			var source = GetElements(size);
			var listPermutation = new List<Permutation>();

			listPermutation.Add(new Permutation(new VectorI(Enumerable.Range(0, size).ToArray()), new VectorI[0]));
			...
			while (listPermutation.Count() < MathUtil.Factorial(size))
			{
				...
				foreach (var elements in listElements)
				{
					...
					{
						listPermutation.Add(new Permutation(vector, elements));
					}
				}
			}

			return listPermutation.ToArray();
		}
